...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * FROM user_form where id=$from";
    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    // $sql = "SELECT id, CONCAT(first_name,' ',last_name) AS fullname,email,balance FROM users where id=$to";
    // $query = mysqli_query($conn,$sql);
    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $newbalance = $sql1['balance'] + $amount;
                $sql = "UPDATE user_form set balance=$newbalance where id=$from";
                $query=mysqli_query($conn,$sql);
                ...
                if($query){
                     echo "<script> alert('Transaction Completed');
                                     window.location='displaytable.php';
                           </script>";
